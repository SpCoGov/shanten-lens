...
mod imp {
    ...
    use windows::Win32::{
        ...
        UI::{
            ...
            WindowsAndMessaging::{
                ...
                GW_HWNDPREV, HWND_TOP, LWA_ALPHA, SWP_NOACTIVATE, SWP_SHOWWINDOW, SW_HIDE,
    ...
    pub fn start(app: &tauri::AppHandle) {
        ...
        thread::spawn(move || {
            ...
            while !stop.load(Ordering::SeqCst) {
                ...
                if current_game
                    .map(|w| !is_live_overlay_target(w.hwnd))
                    .unwrap_or(true)
                {
                ...
                match client_area(game.hwnd) {
                    Some(area) => {
                        ...
                        let regions = panel_regions
                            .lock()
                            .map(|regions| regions.clone())
                            .unwrap_or_default();
                        let visible_region = build_visible_region(
                            game.hwnd,
                            hud_hwnd,
                            area,
                            is_interactive,
    ...
    fn build_visible_region(
        game_hwnd: HWND,
        hud_hwnd: HWND,
        area: ClientArea,
        interactive: bool,
        panel_regions: &[PanelRegion],
    ) -> Option<HRGN> {
        ...
        while let Some(hwnd) = cover {
            ...
            if is_occluding_window(hwnd) {
                if let Some(rect) =
                    window_rect(hwnd).and_then(|r| intersect_rect(r, area.screen_rect()))
                {
                    let relative = RECT {
                    ...
                    if !blocker.is_invalid() {
                        let result = unsafe {
                            CombineRgn(Some(visible), Some(visible), Some(blocker), RGN_DIFF)
                        };
                        unsafe {
    ...
    fn build_panel_region(area: ClientArea, panel_regions: &[PanelRegion]) -> Option<HRGN> {
        ...
        for region in if panel_regions.is_empty() {
            vec![fallback]
        } else {
            panel_regions.to_vec()
        } {
...
pub use imp::{
    get_status, is_overlay_supported, set_enabled, set_interactive, set_panel_regions, start,
    OverlayStatus, PanelRegion,
};
